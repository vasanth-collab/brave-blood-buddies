// =============================================
// DATA STORE - All data operations (using localStorage)
// Think of this as a simple "database" for the app
// =============================================
...
export function searchDonors(filters: {
  ...
  if (filters.availableOnly)
    donors = donors.filter(d => d.isAvailable);
...
// --- Demo Data ---
// Creates fake donors and requests so the app has data to show
export function seedDemoData() {
  if (localStorage.getItem('bloodbank_seeded')) return; // only seed once
  ...
  names.forEach((name, i) => {
    ...
    const donor: DonorProfile = {
      ...
      phone: `98765${43210 + i}`,
      bloodGroup: groups[i], location: locations[i],
      pincode: `${400000 + i * 11}`,
      role: 'donor', isVerified: i < 5,
      createdAt: new Date(now.getTime() - 90 * 86400000).toISOString(),
      isAvailable: i !== 3, // Sneha Patel starts as unavailable
      lastDonationDate: new Date(now.getTime() - daysAgo * 86400000).toISOString(),
      totalDonations: donations,
      totalResponses: Math.floor(donations * 0.8),
      totalRequests: donations + 2,
      reliabilityScore: 0,
  ...
  // Merge with any existing users (don't overwrite real registrations)
  const existing = JSON.parse(localStorage.getItem('bloodbank_users') || '{}');
  localStorage.setItem('bloodbank_users', JSON.stringify({ ...users, ...existing }));
